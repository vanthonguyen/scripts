...
for i in range(0, len(values)) :
    if i == 0:
        ax.plot(values[i], zcs[i], label= label1, linewidth=0.8) 
    elif i == 1 :
        #ax.plot(values[i], zcs[i], '.-', label= "voxel 10cm + 40cm") 
        ax.plot(values[i], zcs[i], label= label2, linewidth=0.8, color='black') 
    else :
        ax.plot(values[i], zcs[i], label= label3, linewidth=0.8) 
