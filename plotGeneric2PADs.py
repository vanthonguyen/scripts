...
ax.set_xlabel("NODATA (%)")
ax.set_ylabel("Height (m)")

for i in range(0, len(values)) :

    if i == 0 :
        ax.plot(values[i], zcs[i], label= "all scans",linewidth=0.8, color=palettes["PAD"])
    elif i == 1 :
        ax.plot(values[i], zcs[i], linestyle='--', label= "center scan",linewidth=0.8, color=palettes["PAD"])
