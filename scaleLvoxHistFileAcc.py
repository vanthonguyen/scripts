...
with open(histFile, 'r') as f:
    for line in f:
        ...
        if nbRows > 0 and nbRows % downscaleFactor == 0:
            outfile.write("%d\t%d\t%f\t%f\t%f\t%f\n"% (nbRows/downscaleFactor - 1, nbRows/downscaleFactor - 1, sumVals[2], sumVals[3], sumVals[4]/downscaleFactor, sumVals[5]/downscaleFactor/downscaleFactor/downscaleFactor))
            sumVals=np.zeros(nbVar)

if nbRows % downscaleFactor != 0:
    nbZ = nbRows % downscaleFactor
    outfile.write("%d\t%d\t%f\t%f\t%f\t%f\n"% (nbRows/downscaleFactor, nbRows/downscaleFactor, sumVals[2], sumVals[3], sumVals[4]/nbZ, sumVals[5]/nbZ/downscaleFactor/downscaleFactor))
