...
for i in range(0, len(values)) :
    if i == 0:
        ...
        ax.plot(values[i], zcs[i], label= "voxel 10cm + interpolation moyenne", linewidth=0.7) 
    elif i == 3 :
        ax.plot(values[i], zcs[i], label= "L-Architect", linewidth=0.7) 
    else :
        ax.plot(values[i], zcs[i], label= "L-Architect with 50% foliage", linewidth=0.7) 
...
outfile = base.split(os.path.sep)[0] + "-10cm40cm.pdf"
fig.savefig(outfile)
